...
interoperability vector below, they must never be persisted, logged, placed in
a receipt or registry, published, or otherwise exposed. Only the typed
`smartkey-g0-hmac-sha256-v1:source_record:<key_id>:<mac>` reference is public.
This remains true even though `raw_sha256` is an approved merged-source digest:
it is distinct from a full reconstructed-file receipt, but it is still a field
inside the private pre-HMAC envelope. Likewise, intentional JSON `null` values
in the public timestamp-authority projection are semantic public values, not
...
the exact ascending tuple `(start_char, end_char, authorship_confidence,
reason)`. After surrogate rejection, Python Unicode-scalar string order is
equivalent to lexicographic UTF-8 byte order, so validation counts UTF-8 width
without materialising or retaining encoded copies. The HMAC boundary requires
that order already; it never silently repairs a fabricated envelope. Exact
duplicate segment objects are invalid, while overlapping non-identical ranges
are allowed. Every malformed source envelope fails non-reflectively as exactly
...
source_record_whole_source_semantic_v1_historical = {
  ...
  "excluded_segments":[
    ...
    {"authorship_confidence":"\ue000","start_char":16,"end_char":20,"reason":"synthetic-zulu"},
    {"authorship_confidence":"\ud800\udc00","start_char":16,"end_char":20,"reason":"synthetic-alpha"}
  ],
