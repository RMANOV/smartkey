"""Phase-A offline analysis + mechanical PASS/FAIL/INCONCLUSIVE gate.

Implements spec §12.4 blockers 1 & 2 verbatim — the gate text is hash-committed
(git 6403096b) and is NOT reinterpreted here, only executed:

  split   : first 50% of resolutions by ts = calibration fit; second 50% =
            scoring. No post-hoc re-splitting.
  buckets : 5 quantile buckets on raw p_top3 over the FIT half; a bucket with
            <30 resolutions in the SCORE half merges into its LEFT neighbour
...
Interpretation note (mechanical disambiguation, documented — not a spec change):
the spec lists PASS and FAIL as independent condition sets that can, in edge
cases, both be satisfiable (e.g. perfect calibration but >5% unresolved). We
evaluate **FAIL first**: a fatal plumbing condition overrides a superficial
calibration pass. This is the only safe deterministic reading.

No LLM / no neural (§12.3): numpy + sklearn IsotonicRegression only.
"""
...
def _quantile_edges(fit_p: np.ndarray, n_buckets: int) -> list[float]:
    ...
    uniq = sorted(set(round(float(e), 12) for e in edges))
    return uniq
...
def _watchdog_status(conn: sqlite3.Connection, first_ts: float, last_ts: float) -> tuple[int, bool]:
    """Return (missed_watchdog_days, alarm_raised)."""
    ...
    missed = len(expected - swept)
    return missed, alarm_raised
...
def analyze(db_path: str | Path, synthetic: bool = False) -> AnalysisResult:
    ...
    res.missed_watchdog, res.alarm_raised = _watchdog_status(conn, first_ts, last_ts)
    conn.close()
    ...
    if n >= 4:
        ...
        # sklearn needs >=1 sample and non-degenerate X; guard degenerate X.
        if len(np.unique(fit_p)) >= 2:
            iso.fit(fit_p, fit_y)
            score_cal = iso.predict(score_p)
        else:
            # degenerate raw signal: calibrated p == base rate of the fit half
    ...
    if res.missed_watchdog > 0 and not res.alarm_raised:
        fail_reasons.append(
            f"missed watchdog {res.missed_watchdog} day(s) without in-band alarm"
        )
    ...
    # --- PASS check ----------------------------------------------------------
    pass_reasons_missing: list[str] = []
    if len(surviving) < MIN_SURVIVING_BUCKETS:
        pass_reasons_missing.append(
            f"only {len(surviving)} surviving bucket(s) (< {MIN_SURVIVING_BUCKETS})"
        )
    if any(b.gap_cal > GATE_PASS_GAP for b in surviving):
        worst = max((b.gap_cal for b in surviving), default=0.0)
        pass_reasons_missing.append(
            f"a bucket has |mean_p-hit_rate|={worst:.3f} > {GATE_PASS_GAP}"
        )
    if res.resolutions < MIN_RESOLUTIONS:
        pass_reasons_missing.append(f"{res.resolutions} resolutions < {MIN_RESOLUTIONS}")
    if res.missed_watchdog != 0:
        pass_reasons_missing.append(f"{res.missed_watchdog} missed watchdog day(s)")
    if res.over_budget_frac > MAX_OVER_BUDGET_FRAC:
        pass_reasons_missing.append(">20ms budget exceeded")

    if not pass_reasons_missing and surviving:
        res.verdict = "PASS"
        res.reasons = ["all surviving buckets within +/-0.05; volume/latency/watchdog clean"]
    else:
        res.verdict = "INCONCLUSIVE"
        res.reasons = pass_reasons_missing or ["insufficient resolved data to bucket"]
    return res


# ---------------------------------------------------------------------------
# Report formatting (report template — ends with the verbatim firewall line)
# ---------------------------------------------------------------------------
def format_report(res: AnalysisResult, db_path: str | Path) -> str:
    L = []
    L.append("=" * 70)
    L.append(f" PHASE-A ANALYSIS REPORT  ({res.scope} data)")
    L.append(f" db: {db_path}")
    L.append(f" freq-table hash (version pin): {res.freq_table_hash[:16]}...")
    L.append("=" * 70)
    ...
    if res.buckets:
        ...
        L.append(
            "  #  raw_p[lo..hi]         n    raw_p̄   base(raw|hit)   cal_p̄   hit    |cal-hit|"
        )
        for b in res.buckets:
            ...
            if b.gap_cal > GATE_FAIL_GAP:
                ...
                flag = "  <-- >0.05"
            L.append(
                f"  {b.index}  [{b.lo:.5f}..{b.hi:.5f}]  {b.n_score:5d}  "
    ...
    L.append(" Phase-A tests the plumbing, not the model). A green result is")
    L.append(" inadmissible as team-tier proof; a red result kills/redesigns the plumbing.")
    L.append("")
    L.append(FIREWALL_LINE)
    return "\n".join(L)


def main() -> int:
    ap = argparse.ArgumentParser(description="Phase-A analysis + gate")
    ...
    if args.json:
        ...
        # firewall line is still required in every report:
        print(FIREWALL_LINE)
    else:
        print(format_report(res, args.db))
    # exit code encodes verdict for cron/scripting: 0 PASS, 2 FAIL, 3 INCONCLUSIVE
