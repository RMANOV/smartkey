"""Offline calibration analysis with a mechanical PASS/FAIL/INCONCLUSIVE gate.

For real campaigns the input population comes from the same canonical selector
as the reconciliation report. Synthetic self-tests retain their isolated path.
...
FAIL-first precedence means a fatal plumbing condition overrides a superficial
calibration pass. Operational HOLD flags can never produce PASS.
...
def analyze(
    ...
    res = AnalysisResult(
        ...
        scope=scope,
        freq_table_hash=freq_table_hash,
    ...
    if fail_reasons:
        res.verdict = "FAIL"
        res.reasons = [*(f"HOLD: {flag}" for flag in res.hold_flags), *fail_reasons]
        return res
    ...
    pass_missing.extend(f"HOLD: {flag}" for flag in res.hold_flags)
...
def format_report(res: AnalysisResult, db_path: str | Path) -> str:
    ...
    if res.scope == "real":
        ...
        if res.hold_flags:
            L.append(f" HOLD flags           : {'; '.join(res.hold_flags)}")
