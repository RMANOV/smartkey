"""SQLite event-logging harness for O1 / Phase-A (spec §12.4 blocker-3 schema).
...
Blocker-3 target schema (spec §2):
    { ts, context_hash, top3, p_top3(raw normalized freq), latency_us,
      resolved_token, outcome in {0,1}, class='machine',
      resolver='script:next_token_in_top3' }

PRIVACY divergence (deliberate, documented — see README-O1 "Deviations"): the
gate needs only ``outcome in {0,1}`` and ``p_top3``. Plaintext ``top3`` and
...
``context_hash`` instead. This honors the spec's "never live events.db /
identity / no keystroke logs" invariant (§4/§8) at the storage layer.

The no-LLM schema invariant (G-FEAT §6-ii) is enforced structurally: a CHECK
