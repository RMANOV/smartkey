"""End-to-end machinery self-test for the O1 / Phase-A n-gram calibration
harness (spec §12.4 F: build-gate self-test incl. forced-FAIL).
...
Checks:
  ...
  10 CORPUS PIN : every pinned corpus file matches its frozen sha256.
...
def check_corpus_pin() -> tuple[bool, str]:
    seen = verify_lab_corpus()
    ok = all(seen.get(n) == h for n, h in PINNED_CORPUS_SHA256.items())
    files = lab_corpus_files("json")
    ok = ok and len(files) == 3 and all(f.parent == LAB_CORPUS_DIR for f in files)
