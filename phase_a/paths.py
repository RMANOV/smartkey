"""Lab-local paths + hard isolation guards for O1 / Phase-A artifacts.

Two invariants, enforced structurally (fail-closed), per the task freeze:
  ...
  2. Corpus inputs are ONLY the pinned copies in ``<worktree>/lab_corpus/``.
     ``lab_corpus_files()`` verifies each file's sha256 against the frozen pins
...
LAB_ROOT = Path(__file__).resolve().parent.parent  # the o1-lab worktree root

# --- Pinned corpus (task freeze; verified at start and end of every run) -----
...
def verify_lab_corpus() -> dict[str, str]:
    """Verify every pinned corpus file matches its frozen sha256. Returns the
    {name: sha256} map on success; raises RuntimeError on the first mismatch
    (fail-closed: the harness must not run on substituted corpus data)."""
    seen: dict[str, str] = {}
    for name, pinned in PINNED_CORPUS_SHA256.items():
        p = LAB_CORPUS_DIR / name
        if not p.exists():
...
def lab_corpus_files(kind: str = "json") -> list[Path]:
    ...
    return sorted(p for p in (LAB_CORPUS_DIR.glob(f"corpus_*{suffix}")))
