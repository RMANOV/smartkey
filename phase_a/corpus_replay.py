"""O1 lab driver: generate Phase-A resolution events from the PINNED corpus.
...
own bigram distribution as the ground-truth next-token stream:
  ...
  * the realized next token is sampled from ctx's follower distribution
    ...
    split, it does not grade the model — spec §5);
...
then overshoots the SCORING half by > 0.10 in >= 3 buckets -> the gate goes RED
via the §5 calibration-gap FAIL clause. This is the injected miscalibration for
the mandatory forced-FAIL receipt; it is NEVER used in a clean gate run.
