"""O1 live-run collector — passive ngram-component telemetry shim.

Design artifact (Δ1–Δ4 frozen): ``docs/O1-SHIM-DESIGN-2026-07-19.md``.
Frozen spec: O1-P1-LIVE-RUN-SHIM-SPEC-2026-07-19.md sha256 68a343b463b6….
...
performs zero DB I/O (Δ2). The DB lives in a config-driven data root that is
guarded fail-closed against the live/operator trees BEFORE any open (A.3),
via ``phase_a.paths.data_dir()``. The HMAC context salt is the 0600 sidecar
from ``phase_a.paths.context_salt()`` — outside the DB, outside any repo or
push path, never logged (Δ1).

Failure isolation (A.4): any collector error disables the tap and never
...
class O1Collector:
    ...
    def __init__(
        ...
        # A.3: resolve + guard the data root BEFORE any DB open. phase_a's
    ...
    def on_word_commit(self, word: str) -> None:
        ...
        try:
            ...
            # Δ2 frozen interval: before the FFI call → after the row
    ...
    def _put(self, entry: tuple) -> None:
        try:
            ...
            self._dropped += 1  # counted; surrounding window excluded per Δ3
    ...
    def _writer_run(self) -> None:
        ...
        try:
            while True:
                ...
                if batch:
                    conn.commit()
                if self._kill_file.exists() and self._active:
                    self._activate_kill("writer cycle")
