"""O1 shim focused tests — T1–T8 from docs/O1-SHIM-DESIGN-2026-07-19.md §8.

Safety: no real IBus, no ``smartkey_py`` native module, no live/Phase-A data —
collector tests run against a contract-faithful fake core in an isolated
``SMARTKEY_PHASEA_DATA`` temp root per test. End-to-end FFI parity against the
real Rust core is pinned two ways: the Rust unit tests in
``crates/smartkey-core/src/o1_shim.rs`` and T2 here evaluate the SAME shared
truth table (identical inputs → identical pinned expectations); the gate's
independent exact-HEAD rebuild exercises the FFI itself.
"""
...
def test_t1_schema_privacy(tmp_path):
    ...
    # Δ1: salt sidecar exists, is 0600, outside the DB, and never in logs.
...
# ---------------------------------------------------------------------------
# T8 — Δ3 reconciliation over a crafted DB covering every reason code.
# ---------------------------------------------------------------------------
